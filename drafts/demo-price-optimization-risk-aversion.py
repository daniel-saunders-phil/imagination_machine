...
app = marimo.App(app_title="A Bayesian decision theory workflow")
...
def _():
    ...
    prices_ = build_ar_process(noise=0.1,)
    prices = np.convolve(prices_, np.ones(7)/7, mode='valid')
    ...
    return az, np, pm, prices, pt, pytensor
...
def _(mo):
    mo.md(r"""
    To find the optimal price, we need to do a few things. Each of the things shows off a different way to reuse-while-modifying model components. I should mention, I borrowed heavily from this [workshop](https://www.youtube.com/watch?v=boB16AX0OLw) by Ricardo Vieira and Jesse Grabowski[^6].

    [^6]: Since originally drafting, I've learned of a number of other resources on optimizing with PyMC models. [Ravin Kumar](https://www.youtube.com/watch?si=DzDYJQ4_ItFDT1mt&v=WT7o8QRIiQI&feature=youtu.be), [Thomas Wiecki and Ravin ](https://twiecki.io/blog/2019/01/14/supply_chain/), [Jesse and Ricardo again](https://www.youtube.com/watch?v=1SjOYFW67rg).

    1. Build a profit function. Right now, we only have a function for demand. But it is no good optimizing for demand though. That's trivial - set price to 0 and the goods will fly off the shelf. To calculate profit, we need to introduce some notion of unit cost.
...
def _(model, pt):
    ...
    profit_objective = -1 *unit_profit * model.mu
...
@app.cell
def _(pt, posterior_objective):
    #| echo: true
    expected_objective = pt.mean(posterior_objective)
...
def _(model, pt):
    ...
    return objective, optimizable_x
...
@app.cell
def _(objective, optimizable_x, pt, pytensor):
    #| echo: true
