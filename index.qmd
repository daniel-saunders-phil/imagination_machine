...
{
  // ── Sizing ───────────────────────────────────────────────────────────
  const HEIGHT = Math.floor(window.innerHeight * 0.66);
  ...
  // ── Phase 1 · leapfrog trajectory (200 ms/step × 23 steps ≈ 4 s) ─────
  ...
  (async () => {
    while (alive) {
      for (let s = 0; s < 25 && alive; s++) {
        ...
        if (alive) await Promises.delay(1000);  // 1 s before next trajectory
      }
    }
  })();
