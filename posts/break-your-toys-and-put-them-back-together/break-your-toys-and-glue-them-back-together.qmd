...
date: 2023-09-13
description: "More science, even less p-values."
...
with pm.Model() as m0:
    ...
    mu = times * (-a + b*(np.euler_gamma + np.log(N)))
...
```{python}
with m0:
    trace0 = pm.sample(nuts_sampler="nutpie",progressbar=False)
```
...
```{python}
az.plot_pair(trace0,var_names=["a","b"]);
```
...
predictions = post_pred_m0.posterior_predictive['Y'].values.reshape((4000,10))
...
```{python}
G = nx.DiGraph()
island_labels = np.concatenate((np.array(["Papua New Guinea"]),dk.culture.values))
G.add_nodes_from(island_labels)
G.add_edges_from([("Papua New Guinea","Manus"),
                  ("Papua New Guinea","Trobriand"),
...
```{python}
with m1:
    trace1 = pm.sample(nuts_sampler="nutpie",progressbar=False)
```
...
predictions = post_pred_m1.posterior_predictive['Y'].values.reshape((4000,10))
label_id = dk.culture.values
...
```{python}
trace0 = pm.compute_log_likelihood(trace0,model=m0)
trace1 = pm.compute_log_likelihood(trace1,model=m1)
