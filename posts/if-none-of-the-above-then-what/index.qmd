...
A couple days ago, Richard McElreath had a lovely [blog post](https://elevanth.org/blog/2023/07/17/none-of-the-above/) on why the Bayes factor isn't the cure to what ails us. No p-values or No confidence intervals, either. No metric of any kind can replace the need for clear model-first thinking as the justification for scientific claims. A common reaction to Richard's post is to ask the obvious question, "if none of the above, then what?" The question underscores just how difficult it is to imagine another kind of science. 
...
post_pred_m0 = az.from_json('Oceania_posterior_predictions')
